...
class GameProblem(SearchProblem):
    ...
    def actions(self, state):
        ...
        if (state[0][0] + 1 >= self.CONFIG['map_size'][0] or 
            self.getAttribute((state[0][0] + 1, state[0][1]), 'water') or
            (state[2] - self.getAttribute((state[0][0] + 1, state[0][1]), 'cost') <= 0)):
        ...
        if (state[0][1] + 1 >= self.CONFIG['map_size'][1] or
            self.getAttribute((state[0][0], state[0][1] + 1), 'water') or
            (state[2] - self.getAttribute((state[0][0], state[0][1] + 1), 'cost') <= 0)):
    ...
    def result(self, state, action):
        ...
        if action == 'North':
            newState = ((state[0][0], state[0][1] + 1), state[1], state[2])
        elif action == 'East':
            newState = ((state[0][0] + 1, state[0][1]), state[1], state[2])
        elif action == 'South':
            newState = ((state[0][0], state[0][1] - 1), state[1], state[2])
        elif action == 'West':
            newState = ((state[0][0] - 1, state[0][1]), state[1], state[2])
        ...
        if (self.getAttribute((newState[0][0], newState[0][1]), 'isGoal')) and ((newState[0][0], newState[0][1]) not in list(newState[1])):
            ...
            newState = ((newState[0][0], newState[0][1]), newStateGoalTuple, newState[2])

        #Updating battery value on each step...
        newStateList = list(newState)
        newStateList[2] = newStateList[2] - self.getAttribute(newState[0], 'cost')
        newState = ((newState[0][0], newState[0][1]), newState[1], newStateList[2])

        #Checking if we step on a charge station...
        if self.getAttribute(newState[0], 'charge'):
            newStateList = list(newState)
            newStateList[2] = self.CONFIG['battery']
            newState = ((newState[0][0], newState[0][1]), newState[1], newStateList[2])
    ...
    def is_goal(self, state):
        ...
        '''
        return (sorted(state[1]) == sorted(self.GOAL[1])) and (self.AGENT_START == (state[0][0], state[0][1]) and (state[2] >= 0))
    ...
    #Trying to get a list of currently not visited goals...
        ...
        #First heuristic based on relative distances beetween our drone and its base/goals...
        h = (goalDistance / 2) + (baseDistance / 2) + (self.getAttribute((state[0][0], state[0][1]), 'cost'))

        #Alternate heuristic based in pythagorean theorem...
        goalDistance1 = 0
        baseDistance1 = 0

        for x in lgoal:
            goalDistance1 = goalDistance1 + sqrt((x[0] - state[0][0])**2 + (x[1] - state[0][1])**2)

        baseDistance1 = sqrt((state[0][0] - self.AGENT_START[0])**2 + (state[0][1] - self.AGENT_START[1])**2)

        h1 = (goalDistance1) + (baseDistance1) + (self.getAttribute(state[0], 'cost'))

        #You can choose between heuristics using 'h' or 'h1' respectively...
        return h1

    def setup (self):
        initial_state = (self.AGENT_START, (), 100)
